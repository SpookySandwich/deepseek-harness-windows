...
; 6.5 Components page: optional features (dsh CLI on PATH)
!define MUI_PAGE_CUSTOMFUNCTION_PRE SkipIfPassive
...
; Optional component (selected by default): put the bundled dsh CLI on PATH.
...
; Optional component (selected by default): pre-install the dsh-smooth-stream
; plugin. NSIS only drops a marker; the shell merges the bundled payload into
; the user's web profile on next start (JSON merge is a poor fit for NSIS).
Section "Install dsh-smooth-stream plugin (smooth streaming output)" SecSmoothStream
  CreateDirectory "$INSTDIR\resources\runtime\plugins"
  FileOpen $0 "$INSTDIR\resources\runtime\plugins\install-smooth-stream" w
  FileWrite $0 "1"
  FileClose $0
SectionEnd

!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
  !insertmacro MUI_DESCRIPTION_TEXT ${SecDshPath} "Adds a dsh.cmd launcher (bundled Node + @deepseek-ai/dsh) to PATH so you can run dsh in a terminal."
  !insertmacro MUI_DESCRIPTION_TEXT ${SecSmoothStream} "Pre-installs the dsh-smooth-stream plugin into the web profile: assistant replies fade in by paragraph with smooth scrolling."
...
Section Uninstall
  ...
  RMDir "$INSTDIR\bin"
  Delete "$INSTDIR\resources\runtime\plugins\install-smooth-stream"
  RMDir "$INSTDIR\resources\runtime\plugins"
